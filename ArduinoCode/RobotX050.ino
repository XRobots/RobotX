...
#define OFFSET1  2040;  // 2050 right ankle - lower value tips inwards
#define OFFSET2  2030;  // 1930 right lower leg - lower value is more bent
#define OFFSET3  2315;  // 2300 right upper leg - lower value is more bent
#define OFFSET4  2050;  // 2070 right hip - higher moves in
#define OFFSET5  2450;  // 2400 left ankle - higher value tips outwards
#define OFFSET6  2180;  // 2080 left lower leg
#define OFFSET7  2380;  // 2280 left upper leg
#define OFFSET8  2145;  // 2165 left hip - higher moves in
...
double Pkc = 30;   // IMU 3 - body side to side
double Ikc = 0;
double Dkc = 0.5;

double Pk100 = 5;      // Back/front servo
double Ik100 = 0;
double Dk100 = 0;
 ...
 if (currentMillis - previousMillis >= interval) {  //start timed event
   ...
   //Serial.println(IMUtrack01);
    ...
    fbStab = Output101/2;   // *************** front to back stability stuff
        ...
        if (state01 == 0 && stateRight == 0 && currentMillis - previousStepMillis >= 700  ) { 
        ...
        if (state01 == 1 && stateLeft == 0 &&  currentMillis - previousStepMillis >= 700  ) { 
            previousStepMillis = currentMillis;
    ...
    if (stateLeft == 0) {  
              ...
              if (stateLeft == 2 && Input2 < -200 && Input3 < -200 && IMU3b > -0) {  // if leg is bent
                stateLeft = 0;            
            }
    ...
    if (stateRight == 0) {
                ...
                if (stateRight == 2 && Input6 < -200 && Input7 < -200 && IMU3b < 0) {  // if leg is bent
                  stateRight = 0;
                  // probably set the next state to 3.
                  // then make another state that kicks forward if the button is pressed, or just sets the state back to zero if it's not.                                                      
                }
    ...
    if (but1 == 0) {                 //   first toggle switch - manual mode 
        ...
        if (but6 == 1) {
          ...
          Setpoint4 = 0;
            ...
            Setpoint1 = 0 + (Outputc/1.5); // ankle
            Setpoint5 = 0 - (Outputc/1.5);  // ankle
            Setpoint8 = 0 + Outputc;
            Setpoint4 = 0 - Outputc;
