...
struct RECEIVE_DATA_STRUCTUREI2C{
  //put your variable definitions here for the data you want to send
  //THIS MUST BE EXACTLY THE SAME ON THE OTHER ARDUINO
...
#define OFFSET1  2100;  //  2060 right ankle - higher value tips robot inwards
...
double Pka = 10;   // IMU 1 - left leg hip
double Ika = 0;
double Dka = 0.1;
...
double Pkc = 25;   // IMU 3 - body side to side
double Ikc = 0;
double Dkc = 0.5;
...
void loop() {
 ...
 if (currentMillis - previousMillis >= interval) {  //start timed event
    ...
    but14 = bitRead(mydata_recI2C.buttons, 14);
  ...
  ET2a.sendData();  // send data back to remote
   ...
   //Serial.println(IMUtrack01);
    ...
    fbStab = Output101/2;   // *************** front to back stability stuff
        ...
        if (state01 == 0 && stateRight == 0 && straightRight == 3  && currentMillis - previousStepMillis >= 430 && IMU3b < 1.5 ) { 
        ...
        if (state01 == 1 && stateLeft == 0 && straightLeft == 3 &&   currentMillis - previousStepMillis >= 430 && IMU3b > -1.5) { 
            previousStepMillis = currentMillis;
            if (but7 == 0) {
              Setpointc = 50;
            }
    ...
    if (stateLeft == 0) {  
              ...
              if (stateLeft == 3 && IMU3b > -2) {                            // wait to cross point before beinging leg down
                stateLeft = 0;
              }                            
    ...
    if (stateRight == 0) {
                ...
                if (stateRight == 3 && IMU3b < 2) {                       // wait to cross point before beinging leg down
                  stateRight = 0;
                }
    ...
    }            // ************************************* end of normal stepping

    else if (but7 == 1) {           // ************************ walking forward     
    ...
    if (stateLeft == 0) {  
              ...
              if (stateLeft == 3 && IMU3b > -1.5) {                            // wait to cross point before beinging leg down
                stateLeft = 0;
              }                            
           

              if (Input2 > -150 && Input3 > -30 && straightLeft == 0) {      // if leg is straight
                straightLeft = 3;                                            // set straight flag
              }
    ...
    if (stateRight == 0) {
                ...
                if (stateRight == 3 && IMU3b < 1.5) {                       // wait to cross point before beinging leg down
                  stateRight = 0;
                }
 
                if (Input6 > -150 && Input7 > -30 && straightRight == 0) {   // if leg is straight
                  straightRight = 3;                                        // set straight flag
                }
    
               
     
    }                               // end of walking forward
    ...
    if (mode == 2) {
      Serial.println(mydata_recI2C.axis1);      
    
    
    mydata_arms.rot1_left = mydata_recI2C.axis1;
